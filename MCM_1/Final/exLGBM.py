...
# 读取数据
df = pd.read_csv('prc_pretrain_dicreate.csv', encoding='gbk')
X = df.iloc[:, 1:32]
y = df.iloc[:, 0]
...
def Predict_One(filepath):
    ...
    y_pred_new = m1.predict(X_new.iloc[:, 1:32])
    ...
    result_df.to_csv('res_dicreate_backup.csv', index=False, encoding='gbk')  # 请替换为输出路径

Predict_One('prc_pretrain_dicreate.csv')
