...
class WeightedLasso:
    def __init__(self, X, y, weight, lam):
        self.model = LinearRegression(X.shape[1], 1).to(device)
        self.X = X.to(device)
        self.y = y.to(device)
...
class McModel:
    def __init__(self, num_classes, X, y):
        ...
        self.X = X.to(device)
        self.y = y.to(device).reshape(-1, 1)
        self.center = None
        self.bias = None
        self.domain = None
        self.weights = None
    ...
    def cluster(self, weight, past_domains, reuse=False):
        self.center = torch.zeros((self.num_classes, self.X.shape[1]), dtype=torch.float32, device=device)
        self.bias = torch.zeros(self.num_classes, dtype=torch.float32, device=device)

        if past_domains is None or not reuse:
            self.domain = torch.randint(0, self.num_classes, (self.X.shape[0],), device=device)
        else:
            self.domain = past_domains.to(device)
        ...
        while not end_flag:
            ...
            diff = (self.domain != new_domain).float()
            delta = torch.sum(diff).item()
            if iter % 10 == 9:
