...
class LinearRegression(nn.Module):
    def __init__(self, input_dim, output_dim=1):
        ...
        self.linear = nn.Linear(input_dim, output_dim, bias=False).to(device)
        self.weight_init()
...
class FeatureSelector(nn.Module):
    def __init__(self, input_dim, sigma):
        ...
        self.sigma = sigma
        self.mu = torch.nn.Parameter(0.00 * torch.randn(input_dim, device=device), requires_grad=True)
        self.noise = torch.randn(self.mu.size(), device=device)

    def renew(self):
        self.mu = torch.nn.Parameter(0.00 * torch.randn(self.input_dim, device=device), requires_grad=True)
        self.noise = torch.randn(self.mu.size(), device=device)
...
class MpModel:
    def __init__(self, input_dim, output_dim, sigma=1.0, lam=0.1, alpha=0.5, hard_sum=1.0, penalty='Ours'):
        self.input_dim = input_dim
        self.backmodel = LinearRegression(input_dim, output_dim).to(device)
        self.loss = nn.MSELoss()
        self.featureSelector = FeatureSelector(input_dim, sigma).to(device)
        ...
        self.accumulate_mip_penalty = torch.zeros(10, dtype=torch.float32, device=device)
    ...
    def combine_envs(self, envs):
        ...
        y = []
        for env in envs:
            X.append(env[0].to(device))
            y.append(env[1].to(device))
    ...
    def single_iter_mip(self, envs):
        ...
        penalty = torch.zeros(self.input_dim, dtype=torch.float32, device=device)
        ...
        total_loss = loss_avg + self.alpha * penalty_detach + self.lam * reg
